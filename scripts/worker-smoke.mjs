...
try {
 ...
 const cookie=page.headers.get('Set-Cookie').split(';')[0];const csrf=cookie.split('=')[1];
 const accepted=await fetch(origin+'/authorize?'+params,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded',Cookie:cookie},body:new URLSearchParams({csrf,password}),redirect:'manual'});assert.equal(accepted.status,302);
 ...
 const refresh={grant_type:'refresh_token',client_id:client.client_id,refresh_token:tokens.refresh_token};
 ...
 console.log('PASS: 真实本地 workerd + SQLite DO，OAuth / 原子防重放 / MCP 31 工具发现 / 默认只读；无真实云 API 调用。');
